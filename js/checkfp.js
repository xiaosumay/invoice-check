$("#checkfp").off('click').click(function () {
    ...
    if (dmchek.length > 0) {
        ...
        if (aur()) {
            ...
            if (avai(fplx)) {
                ...
                var sjip = dmchek[1];
                url = sjip + "/vatQuery";
                param = { 'key1': fpdm, 'key2': fphm, 'key3': kprq, 'key4': kjje, 'fplx': fplx, 'yzm': yzm, 'yzmSj': yzmSj, 'index': jmmy, 'area': area, 'publickey': $.ck(fpdm, fphm, kprq, kjje, yzmSj, yzm) };
                if (oldweb == 1) {
                    url = sjip + "/invQuery";
                    param = { 'fpdm': fpdm, 'fphm': fphm, 'kprq': kprq, 'fpje': kjje, 'fplx': fplx, 'yzm': yzm, 'yzmSj': yzmSj, 'index': jmmy, 'area': area, 'publickey': $.ck(fpdm, fphm, kprq, kjje, yzmSj, yzm) };
                }
                delayMessage = "发票查验请求失败!";
                showTime();
                $.ajax({ type: "post", url: url, dataType: "jsonp", data: param, jsonp: "callback", success: function (jsonData) {
                    delayFlag = "1";
                    ...
                    if (cyjgdm == "1") {
                        ...
                        if($("#screenshot").length === 0 ){
                            $('#dialog-body').parent()
                                .append('<button id="screenshot" style="margin-top: 1rem; background-color: #015293; color: white; float: right;">截图</button>');
                        }

                        $('#dialog-body').parent().on('click', '#screenshot', function() {
                            var lefttop = $(this).parent().offset();
                            var width = $(this).parent().width() + 17 * 2;
                            var height = $(this).parent().height() + 17 * 2;

                            console.log(JSON.stringify([lefttop.left, lefttop.top, width, height]));

                            jsmgr.screenshot(lefttop.left, lefttop.top, width, height);
                        });
                    }
                    else if (cyjgdm == "002") {
                        show_yzm = "";
                        ...
                        show_yzm = "";
                        setText = "不一致";
                        param = { 'fplx': fplx, 'swjg': swjgmc, 'fpdm': fpdm, 'fphm': fphm, 'kprq': kprq, 'kjje': kjje, 'cysj': yzmSj, 'setText': setText, 'key2222': key2222, 'key3333': key3333 };
                        if (browser == "edge" || browser == "firefox") {
                    ...
                    $('#uncheckfp').hide();
                    $('#checkfp').show();
                }, timeout: 30000, error: function (XMLHttpRequest, textStatus, errorThrown) {
                } });
            }
        }
    }
